...
journal=pd.read_csv(Path('D:\docs\Masters\Data\jpe_refs2.csv'))
path=Path('D:\docs\Masters\Data\JPE_data\JPE_shards')
#year=1946
#ofile_name='abc'+str(year)+'.csv'

# alternative using year range
year=[1947, 1976]
ofile_name='abc'+str(year[0])+'_'+str(year[1])+'.csv'
...
journal['fp']=path/journal['pdf_url'].str.split('/').str[-1].str[:-1]
...
len(journal['id'].unique())

ids=pd.DataFrame(journal[['id','year']].value_counts()).sort_values('year', ascending=False)
